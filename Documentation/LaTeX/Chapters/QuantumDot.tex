\chapter{The Quantum Dot} \label{sec:QuantumDot} \index{Quantum Dot}
	...
	term is more general and includes also other shapes\index{Shape} and morphologies\index{Morphology}, which might extend to several $\mu$m, such as rods, wires etc.
	...
	\paragraph{Basic physics of the \glspl{QD} }
		For bulk materials\index{Bulk Material} the band gap\index{Band gap} is a fixed material parameter. But when a particle gets smaller and reaches
		...
		This phenomenon is called the {\it quantum size effect}\index{Quantum Size Effect}.
		...
		We will see in chapter \ref{sec:BandGapAnalysis} that the \gls{QD} absorption spectrum, \index{Absorption spectrum} as shown in figure \ref{fig:QDTheory}(c), is not really discrete. Since it is
		not possible to fabricate \glspl{QD} that are perfectly equal in size, this results in a broadening of the spectrum\index{Spectrum}\index{Spectrum!Broadening}.
		...
		This is caused by Heisenberg's uncertainty principle \index{Heisenberg uncertainty principle}, which states, that if we want to locate a particle of effective mass $m$ (for example an electron),
		...
		\end{equation}
		This adds to the kinetic energy of the free particle, which is called the confinement \index{Confinement} energy\index{Confinement!Energy}. It has an significant impact, if it gets bigger
		...
		\end{equation}
		Table \ref{tbl:ConfinedStr} shows four possible confinement structures\index{Confinement!Structure}, which are illustrated in Figure \ref{fig:ConfinedStr} with their
	...
	\paragraph{Fabrication techniques of \gls{QD}}\index{Fabrication Techniques of QDs}
	...
	\paragraph{Applications}\index{Applications of QDs}
